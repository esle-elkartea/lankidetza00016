package com.code.aon.ui.sales.servlet;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			if (integer != null) {
				...
				criteria.addEqualExpression(customerBean.getFieldName(ISalesAlias.CUSTOMER_ID), integer);
				...
				if (list.size() == 0) {
					criteria = new Criteria();
					criteria.addExpression(customerBean.getFieldName(ISalesAlias.CUSTOMER_REGISTRY_ALIAS),req.getParameter(VALUE_PARAMETER));
					list = customerBean.getList(criteria);
					if (list.size() > 0) {
						ILookupObject ito = (ILookupObject) list.get(0);
						renderResponse(res, ito, ids);
